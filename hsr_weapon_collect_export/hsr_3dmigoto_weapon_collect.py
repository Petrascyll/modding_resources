...
# Modified by _scyll to support dumping certain hsr weapons
    ...
    for i in range(len(model_data)):
        ...
        # Some objects do not have a separate vb1, and all the data is contained in vb0
            ...
            if args.no_blend or (not args.force_object and not point_vb_candidates):
                ...
                # Some hsr weapons have a singular blend index per vertex with no blend weights, e.g. Herta Hammer, Lynx Pickaxe
                # Singular Blend Index, but No Weights => Weight is implied == 1 for the sole index. We'll make it explicit so the import/export script doesn't freak out
                if blend_format == [{'semantic_name': 'BLENDINDICES', 'element_name': 'BLENDINDICES', 'index': '0', 'format': 'R32_UINT', 'bytewidth': 4}]:
                    ...
                    print()
                    print("BLENDINDICES has only 1 index (R32_UINT), and no BLENDWEIGHTS exist.")
                    print("This is sometimes the case for hsr weapons. (e.g. Herta Axe, Lynx Pickaxe, ...)")
                    print("Manually inserted blend weight = 1 for each vertex to compensate.")
                    print()
